...
The owner suspended the automated pass of gitar (D-471). Do not ask for a gitar review, and do not wait for one.

- When gitar posts a comment on a PR, the author answers it before the hand-over or the override request. The `gitar-review` skill holds the procedure (D-374).
- The reviewing provider never addresses gitar (`pr-review`).
- A reply names no provider, harness, or model as the source of work (T-6).
- `.claude/skills/gitar-review/references/restore.md` holds the original rule and the restore steps.
...
- [ ] Every comment that gitar posted has its answer. The owner suspended the pass, so no gitar review is necessary (D-471).
