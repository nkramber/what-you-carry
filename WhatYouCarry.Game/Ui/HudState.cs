...
/// remain and whether it expired (D-443), whether the stairwell prompt is open, whether it offers the descend (D-579),
/// and whether the controller gave the last input (D-447).
/// </summary>
/// <remarks>
...
/// </remarks>
public readonly record struct HudState(int Health, int MostHealth, long RemainingTicks, bool Expired, bool PromptOpen, bool DescendOffered, bool Controller)
{
    ...
    {
        return new HudState(loop.Player.Health, Player.MaxHealth, loop.Timer.Remaining, loop.Timer.Expired, StairwellPrompt.IsOpen(loop), StairwellPrompt.OffersDescend(loop), controller);
    }
}
