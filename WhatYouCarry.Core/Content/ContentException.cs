...
/// <summary>
/// The error that a content failure raises. It names the file, the field, and the reason (D-92, T-2).
/// </summary>
public static class ContentError
{
    ...
    {
        ContextException error = new($"The content file '{path}' is not valid. The field '{field}' {reason}.");
    ...
    {
        ContextException error = new($"The content file '{path}' is not valid. {reason}.");
        error.AddContext("file", path);
        error.AddContext("reason", reason);
        return error;
    }
