...
| D-202 | 2026-09-08 | Lint parser | The `det-lint` tool parses each Core source file with `Microsoft.CodeAnalysis.CSharp`, the C# compiler API. | Resolves OQ-75. Binds PR-3. This is the dependency entry that G-16 requires. D-67 named the Core syntax trees and no package. A parse tells an identifier from a comment and from a string, so a banned name in prose gives no finding. A parse also reads a name behind a `using static`, which a text search cannot see. The package is the compiler that the SDK already ships, and it stays in the tool project. |
