...
| D-172 | 2026-09-07 | Attribution option | `.claude/settings.json` in the repository sets `attribution.commit` and `attribution.pr` to false, verified against the Claude Code settings reference on 2026-09-07. Codex is unverified. The PR-1 trailer scan is the backstop. | Resolves OQ-16 and F-15 for Claude Code. Revised by D-175 on 2026-09-07: empty strings, not booleans. |
...
| D-179 | 2026-09-07 | Review gate check | A `review-gate` CI job reads `docs/reviews/pr-<number>.md` for the PR number. It fails when the file is absent, when the verdict is not `Ready for owner merge`, or when the head that the file records is not the effective head. The effective head is the newest commit that changes a path outside `docs/reviews/`. A review-file commit therefore does not invalidate the review (`pr-review` SKILL.md, metadata commits). | Mechanizes T-4 and D-101. Binds PR-1. The response file stays a convention and the gate does not require it. Revised by D-181 on 2026-09-07: three conclusions, not two. Revised by D-184 on 2026-09-07: the metadata set holds three paths, not one. |
