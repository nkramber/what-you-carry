...
| F-64 | PR #12 review P2-4: the lint tool compared the file name `DetMath.cs` alone, so a second file with that name in another Core directory took the `MathF` exemption. Review P2-3 also found that the reflection rule read the namespace text alone, so `typeof(x).GetMethods()` gave no finding | 2026-09-08 | ✅ Corrected in PR-3. The rule reads the whole Core-relative path, and a reflection member name is a finding on its own |
