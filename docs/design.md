...
The fix of F-111 (D-538). A diagonal drop needs an open fall in the corner column (D-545). An arrival at a waypoint starts the wedge count of `PathFollower` again (D-546). The simulation version rises to 16 (G-20). A night on a branch writes its record to `night-branch/<branch>`, and `main` alone writes `night-results` (D-373, D-538). The `night-gate` job of a PR passes on a success record of its branch night at the effective head or later (D-547). The branch night then re-runs the gate of its PR (D-548).
