...
- One open item waits for the owner. PR-4 states a Core contract that no decision holds: a field name is an identifier and must hold valid text, and a value and a message take the replacement character. The response file, the design doc, the roadmap, and the code carry it. The owner can make it a decision.
...
The owner merges this documentation PR with the `review-override` label. Then a new session starts PR-5: the content loader, the schemas, the content hash, and the string table (D-91, D-92, D-98, D-163, D-168).
