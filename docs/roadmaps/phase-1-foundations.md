...
Status: merged 2026-09-09 as PR #29, commit `5ae0a24`.
...
Status: merged 2026-09-10 as PR #31, commit `1ce78c6`.

Scope:

- `content/projectiles/`: a test-only set of projectile definitions, one file each (F-38, D-149): the slowest arc, the fastest flat shot, the longest lifetime, and the widest spread. Real weapons arrive in PR-24 and PR-43 to PR-46, and rerun these tests.
- The projectile schema gains the required field `spreadHundredths`, the half angle of the spread cone in hundredths of a degree (D-266). The Projectile stream of D-159 draws the spread of each shot.
...
5. `ProjectilesAreDeterministic` replays a record with shots and asserts one state hash, and the `bit-identity` job asserts it on three platforms. Revised 2026-09-13 in PR-15 by D-320: no intent fires a shot, so the test fires through the simulation, and the sweep folds a projectile run of its own.
...
- Policies in Core, each a few dozen lines (D-149): `RandomWalker` holds a random movement and jump for a random number of ticks, then picks again. `GreedyDescender` walks the reachability path to the stairwell and always descends. A policy draws from the new `RngStream.Bot` value of its run seed, the fifth value of D-159, and never from a simulation stream (D-272). The bit-identity sweep lists its streams by name, so the new value moves no hash until a later change adds it there.
- Run end states (D-270). `bottom`: the run ascends at the stairwell of floor 15. `budget`: the tick budget of a policy that promises no progress runs out. `softlock`: a floor budget passes with no floor progress on a policy that promises progress. `crash`: an exception, and the log holds it. The random walker ends by budget, and the greedy descender by bottom or softlock.
...
6. The night job completes ten thousand runs with zero crashes, and zero softlocks on the greedy descender (D-270).
...
Status: merged 2026-09-10 as PR #40, commit `5bdef87`. Exit tests 1 to 8 passed before the merge, and the job passed on the PR against the record of the second hand run.
...
- The workflow `night-gate.yml` holds one job on Linux with the full history for the ancestry check. It passes the checkout, the remote, the base ref, and the time to the command. The command fetches `night-results` itself and reads `night.json` from git, never from the working tree of the checkout (F-93). A remote with no such branch, or a branch with no such file, reads as an absent record with the reason. Any other git failure is an error that names the command (T-2).
...
- `night-record` writes the file without a byte-order mark now. `File.WriteAllText` with `Encoding.UTF8` wrote one, and a reader of bytes saw a mark before the object. PR-11 exit test 7 asserts the first byte. The parser accepts a leading mark, because the records of the first two nights carry one.
- The PR gate of the agent files gains the `night-gate` line.
- Beyond the exit tests: `NightGateFailsOnMalformedRecord`, `NightGateReadsTheRecordFromTheRemoteAndNeverFromTheCheckout`, `NightRecordParserAcceptsAByteOrderMark`, `NightGateCommandReportsEachExitCode`, and the shape test `NightGateWorkflowFetchesTheRecordWithFullHistory`.
...
6. Each failure message names the case, the record commit, and the record time (T-2).
7. The `night-gate` job runs on this PR and passes against the real night record (G-19).
8. `NightGateFailsOnForeignCommit` asserts a failure on a fixture record whose commit is not on the base branch (D-275).

Review focus: errors, CI boundaries, test quality.

Check clause: none.

Gate: exit tests 1 to 8 pass, and the job passes on this PR.
...
Status: table complete 2026-09-10, ten PRs (D-276). One job exceeded ten minutes, and D-277 keeps the seed counts of D-116 (OQ-145, F-91).

The rows read the push run on `main` of the squash-merge commit of each PR, one run per PR, on the first attempt. The numbers are the job durations in seconds, from the start of the job to its end. The CI columns are the build-and-test jobs of the CI workflow (D-148). The Bit columns are the platform jobs of the bit-identity workflow (G-9). The Lint, STE, and Bots columns are the single-platform jobs, and a PR before PR-11 has no bots job.

| PR | Merge | CI Linux | CI Windows | CI macOS | Bit Linux | Bit Windows | Bit macOS | Lint | STE | Bots |
|---|---|---|---|---|---|---|---|---|---|---|
| PR-3 | #12, `9e6fd5f` | 37 | 69 | 17 | 23 | 47 | 10 | 17 | 18 | none |
| PR-4 | #15, `1749463` | 36 | 70 | 18 | 22 | 42 | 13 | 29 | 25 | none |
| PR-5 | #17, `e0deb94` | 29 | 76 | 17 | 20 | 46 | 9 | 35 | 24 | none |
| PR-6 | #19, `3f7e3c2` | 43 | 78 | 17 | 22 | 65 | 11 | 24 | 25 | none |
| PR-7 | #21, `d5f20ce` | 46 | 68 | 18 | 21 | 41 | 11 | 26 | 21 | none |
| PR-8 | #23, `a3bdf20` | 39 | 66 | 14 | 11 | 52 | 10 | 20 | 36 | none |
| PR-9 | #27, `336fe4e` | 234 | 253 | 97 | 18 | 37 | 12 | 58 | 14 | none |
| PR-59 | #29, `5ae0a24` | 407 | 274 | 206 | 28 | 36 | 10 | 36 | 20 | none |
| PR-10 | #31, `1ce78c6` | 252 | 601 | 226 | 22 | 33 | 9 | 21 | 23 | none |
| PR-11 | #34, `7487473` | 334 | 574 | 215 | 26 | 68 | 12 | 25 | 21 | 106 |

The CI job grew at PR-9, with the reachability sweep of five thousand seeds, and again at PR-10 and PR-11. The Windows CI job of PR-10 took 601 seconds, one second over the bound of the procedure. The same job of PR-11 took 574 seconds. Every other job stays under seven minutes. The bit-identity, lint, and STE jobs stay under 70 seconds.

### M-2: Night sweep wall time

Procedure: read the night job duration for seven nights: the six hand runs of 2026-09-10 and the first scheduled night of 2026-09-11 (D-278, D-281, D-283). Record them in the table below. If a night exceeds six hours, file a question on the run counts in D-115 and D-116.

| Night | Run | Commit | Minutes | Status |
|---|---|---|---|---|
...
| Scheduled night 1, 2026-09-11 | 34600758086 | `095ce5e` | 58 | success, 4 h 40 min after the 08:07 UTC cron (F-95) |

Status: table complete 2026-09-11, seven nights (D-283). The scheduled night ran 4 h 40 min after its cron, and F-95 read it as a miss at 09:00 UTC. The longest night took 68 minutes against the six-hour bound.
...
17. ✅ PR-59 merged 2026-09-09 as PR #29.
18. ✅ PR-10 merged 2026-09-10 as PR #31.
19. ✅ PR-11 merged 2026-09-10 as PR #34.
20. ✅ Two nights ran by hand on 2026-09-10 (D-278, D-281). The first failed on F-92, and the second passed.
21. ✅ PR-58 merged 2026-09-10 as PR #40.
22. ✅ M-1 table complete 2026-09-10 (D-276, D-277). ✅ M-2 table complete 2026-09-11, seven nights (D-283).
23. ✅ **← GATE 1 (foundation).** Signed 2026-09-11 (D-288). Every exit test in this file passes. The bit-identity job, `dotnet test`, and the night sweep are green. The owner signs the gate in `docs/decisions.md`. Gate 1 signs after the first scheduled night passes on its own (D-283).
...
- OQ-133 to OQ-136 (D-265 to D-268): the Phase 1 shot, the spread field, the fire timing, and the shot origin. PR-10. D-320 supersedes D-265 in PR-15.
...
- OQ-155 (D-286): the schedule test at 17:21 UTC, with the workflow turned off and on. The night workflow and Gate 1.
- OQ-154 (D-288): the Gate 1 sign-off. Gate 1 and Phase 2.
- OQ-156 (D-290): the PR route after the Gate 1 sign-off. The Gate 1 record and the night workflow.
