...
- Still water is not solid (D-258). A pool is a one-block depression in a floor, and the reachability search reads water as air. While the feet stand in a water cell, the body walks and sprints at one half of the speeds of D-231 (D-261). Gravity and the jump velocity take the factor one half there too. The apex stays at one block, and the rise takes twice as long (D-262). The two factors are Core constants in `PlayerBody` (D-263).
