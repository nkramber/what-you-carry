...
- `Core/World/VoxelGrid.cs`: a flat array of one-byte block ids, at most 128 by 32 by 128 (D-78, D-164). Solid or air per id. The frame is the frame of Godot (D-234).
...
- `Core/Entities/PlayerBody.cs`: a box that reads the intent's movement vector, the jump bit, and the sprint bit, applies gravity, and moves through `SweptAabb` (D-149, D-232, D-233). No health, no weapon.
