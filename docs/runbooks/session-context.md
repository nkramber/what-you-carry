...
The `gitar-review` skill holds the procedure and the proof that a review is current (D-374). Start the wait at once after each push, also when gitar paused the automatic reviews (D-575).

```
make gitar-wait PR=<number>
```

The script `.github/scripts/gitar-wait.sh` holds the one shell loop of the wait (D-380):
