...
public sealed class ReplayTests
{
    ...
    public void ReplayReproducesHash()
    {
        ...
        for (int seed = 1; seed <= seeds; seed++)
        {
            ...
            ReplayResult result = RunReplayer.Replay(record, TestWorld.PeacefulContent, new JsonlLogger(logs));
    ...
    public void TornTailTruncates()
    {
        ...
        CollectingSink logs = new();
        ReplayResult result = RunReplayer.Replay(torn, TestWorld.PeacefulContent, new JsonlLogger(logs));
    ...
    public void ATornTailKeepsTheCompleteFrames()
    {
        ...
        ReplayResult result = RunReplayer.Replay(torn, TestWorld.PeacefulContent, new JsonlLogger(new CollectingSink()));
    ...
    {
        MemorySink sink = new();
        sink.Append(RunRecord.WriteHeader(RunRecord.NewHeader(TestWorld.PeacefulContent.Hash, 80UL)));
    ...
    public void ContentMismatchReports()
    {
        ...
        ContextException error = Assert.Throws<ContextException>(() => RunReplayer.Replay(record, TestWorld.PeacefulContent with { Hash = OtherHash }, new JsonlLogger(new CollectingSink())));
        Assert.Contains($"recordContentHash={TestWorld.PeacefulContent.Hash}", error.Message, StringComparison.Ordinal);
    ...
    public void TheReplayVisitsEveryCompleteFrame()
    {
        ...
        RunReplayer.Replay(whole, TestWorld.PeacefulContent, new JsonlLogger(new CollectingSink()), all);
        Assert.Equal(new uint[] { 1, 2, 3, 4, 5, 6 }, all.Ticks);

        TickObserver torn = new();
        RunReplayer.Replay(whole[..(whole.Length - (Intent.FrameSize / 2))], TestWorld.PeacefulContent, new JsonlLogger(new CollectingSink()), torn);
        Assert.Equal(new uint[] { 1, 2, 3, 4, 5 }, torn.Ticks);

        TickObserver none = new();
        RunReplayer.Replay(RunRecord.WriteHeader(RunRecord.NewHeader(TestWorld.PeacefulContent.Hash, 3UL)), TestWorld.PeacefulContent, new JsonlLogger(new CollectingSink()), none);
    ...
    {
        byte[] record = RunRecord.WriteHeader(RunRecord.NewHeader(TestWorld.PeacefulContent.Hash, 3UL));
        ReplayResult result = RunReplayer.Replay(record, TestWorld.PeacefulContent, new JsonlLogger(new CollectingSink()));
